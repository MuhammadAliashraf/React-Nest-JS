...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await postApi("auth/login", form, null, false);
      if (response.success) {
        // Assume API returns { user, access, refresh, roles }
        login(response.data);
        const isAdminUser = response.data.roles?.some(r => ['ADMIN', 'SUPER_ADMIN'].includes(r));
        const redirectTo = searchParams.get("redirect") || (isAdminUser ? "/admin" : "/");
        navigate(redirectTo, { replace: true });
      } else {
        setError(
          response.message?.detail || "Invalid credentials. Please try again.",
        );
      }
    } catch (err) {
      ...
      <div className="max-w-md w-full space-y-12">
        ...
        <div className="text-center space-y-6">
          ...
          <div className="space-y-2">
            <h2 className="text-5xl lg:text-6xl font-black text-black tracking-tighter uppercase leading-none italic">
                Welcome Back
            </h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-8 relative z-10">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between px-1">
                <label className="block text-[10px] font-black uppercase tracking-[0.3em] text-gray-400">
                  Access Code
                </label>
                <Link 
                    to="/forgot-password" 
                    className="text-[9px] font-black uppercase tracking-widest text-gray-300 hover:text-black transition-colors"
                >
                  Forgot Password?
                </Link>
              </div>
            ...
            <div className="space-y-4 pt-4">
                ...
                <div className="flex items-center gap-4 py-2">
                    ...
                    <span className="text-[9px] font-black uppercase tracking-widest text-gray-300">New Recruit?</span>
                    <div className="flex-1 h-px bg-gray-100" />
                </div>
        ...
        <div className="text-center">
            <p className="text-[9px] font-bold text-gray-300 uppercase tracking-[0.2em] leading-loose">
                Encoded with 256-bit encryption.<br />
                By entering, you accept our <Link to="/terms" className="text-gray-400 hover:text-black underline underline-offset-4">Terms</Link> & <Link to="/privacy" className="text-gray-400 hover:text-black underline underline-offset-4">Privacy</Link>.
            </p>
        </div>
      </div>
    </div>
